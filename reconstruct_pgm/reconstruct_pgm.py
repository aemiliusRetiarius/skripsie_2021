...
from cube_gen import get_point_coords, get_uniform_point_coords, gen_dist_df

def get_normalized_error(old_dist, new_dist):
    return abs(new_dist - old_dist) / old_dist

program_path = './build/src/reconstruct_pgm1'

priorPos_path = './Data/working_init_pos.csv'
dists_path = './Data/working_dists.csv'
...
status = subprocess.run([program_path, "--p", priorPos_path, "--d", dists_path, "--r", results_path, "-l", "0.8", "-t", "296", "-i", "30", "-f","true"])
