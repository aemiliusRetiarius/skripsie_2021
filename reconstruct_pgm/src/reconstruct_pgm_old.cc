...
int main(int, char *argv[]) {
  ...
  string dataString = "../dists_inter_5_noise_10%.csv";
  ...
  //pgm1.joint_factor = absorb(pgm1.factors);//->normalize(); //temp
...
void writeResultFile(string fileString, const gaussian_pgm &gpgm)
{
  ...
  fout << ",point_num,x,y,z,x_var,y_var,z_var" <<"\n";
  
  rcptr<SqrtMVG> pointFactorSGPtr;
  string pointRecord = "";
  prlite::ColVector<double> theMn(3);
  prlite::RowMatrix<double> theCov(3,3);
  for(unsigned i = 0; i < gpgm.numPoints; i++)
  {
    ...
    theCov = pointFactorSGPtr->getCov();
    pointRecord = pointRecord + to_string(i)+","+to_string(i+1)+","+to_string(theMn[0])+","+to_string(theMn[1])+","+to_string(theMn[2]);
    //pointRecord = pointRecord + ","+to_string(theCov(0,0))+","+to_string(theCov(1,1))+","+to_string(theCov(2,2))+"\n";
    pointRecord = pointRecord + ","+to_string(100.0)+","+to_string(100.0)+","+to_string(100.0)+"\n";
  }
